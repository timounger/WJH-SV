...
class DoxygenCreator():
    ...
    def add_github_corner(self):
        ...
        if self.s_webside is not None:
            ...
            s_corner_text = None
        if 'PROJECT_LOGO' in self.d_user_settings and  self.d_user_settings['PROJECT_LOGO'] != "":
            s_icon_file = self.d_user_settings['PROJECT_LOGO']
            if '/' in s_icon_file:
                i_index = s_icon_file.rfind('/')
                s_icon_file = s_icon_file[i_index + 1:]
            s_icon_text = S_ICON_FIRST + s_icon_file + S_ICON_LAST
        else:
            s_icon_text = None
